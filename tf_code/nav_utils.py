...
def eval_ap(outputs, global_step, output_dir, metric_summary, N, num_classes=4):
  ...
  """
    ...
    for i in range(logits.shape[1]):
        ...
        ap = ap[0]
        smy_utils.add_value_to_summary(metric_summary, 'aps/ap_{:d}: '.format(i), ap)
        aps.append(ap)
    return aps
...
def eval_dist(outputs, global_step, output_dir, metric_summary, N):
  ...
  """
    ...
    smy_utils.add_value_to_summary(metric_summary, 'dists/success_init: ',
                                   100 * (np.mean(d_inits <= SUCCESS_THRESH)))
    smy_utils.add_value_to_summary(metric_summary, 'dists/success_end: ',
                                   100 * (np.mean(d_ends <= SUCCESS_THRESH)))
    smy_utils.add_value_to_summary(metric_summary, 'dists/dist_init (75): ',
                                   np.percentile(d_inits, q=75))
    smy_utils.add_value_to_summary(metric_summary, 'dists/dist_end (75): ',
                                   np.percentile(d_ends, q=75))
    smy_utils.add_value_to_summary(metric_summary, 'dists/dist_init (median): ',
                                   np.median(d_inits))
    smy_utils.add_value_to_summary(metric_summary, 'dists/dist_end (median): ',
                                   np.median(d_ends))
    smy_utils.add_value_to_summary(metric_summary, 'dists/dist_init (mean): ',
                                   np.mean(d_inits))
    smy_utils.add_value_to_summary(metric_summary, 'dists/dist_end (mean): ',
                                   np.mean(d_ends))
...
def add_default_summaries(mode, arop_full_summary_iters, summarize_ops,
    ...
    s_ops = smy_utils.get_default_summary_ops()

    if mode == 'train':
        s_ops.summary_ops, s_ops.print_summary_ops, additional_return_ops, \
        arop_summary_iters, arop_eval_fns = smy_utils.simple_summaries(
